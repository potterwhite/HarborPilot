...
> **Related:** [English Version →](config_layers.md)
...
| `05_registry.env` | `HAVE_GITLAB_SERVER`、`UBUNTU_SERVER_IP`、`UBUNTU_SERVER_PORT`、`HAVE_HARBOR_SERVER` |
| `06_sdk.env` | `INSTALL_SDK`、SDK 路径模式（完整 URL 依赖 `CONTAINER_NAME`，在 Layer 3 设置） |
...
| `10_serverside.env` | `ENABLE_SERVERSIDE`、`DISTCC_*` 端口、`TOOLCHAIN_DIR`、`PYTHON_VERSION` |
...
# Registry URL（依赖上面设置的 CONTAINER_NAME）
UBUNTU_SERVER_IP="192.168.3.67"
UBUNTU_SERVER_PORT="9000"
REGISTRY_URL="${UBUNTU_SERVER_IP}:${UBUNTU_SERVER_PORT}/team_${CONTAINER_NAME}"

# SDK 路径（依赖 CONTAINER_NAME）
...
# 端口（每个平台必须唯一）
CLIENT_SSH_PORT="2129"
SERVER_SSH_PORT="2130"
GDB_PORT="2365"
```
...
| `project_handover/serverside/serverside_only_entrance.sh` | *（已废弃）* Serverside 管理 |
