...
Layer 2  configs/2_platforms/<platform>.env 平台特有覆盖（只写差异部分）
   ↓
Layer 3  configs/3_hosts/<hostname>.env     主机级覆盖（可选，自动加载，gitignore）
```
...
## Layer 3 — 主机级覆盖（`configs/3_hosts/<hostname>.env`）
...
1. 运行 `hostname` 获取机器名
2. 复制模板：`cp configs/3_hosts/TEMPLATE.env.example configs/3_hosts/$(hostname).env`
3. 编辑文件，取消注释并设置你需要的变量
4. 系统自动加载此文件 —— 无需修改脚本

详见 `configs/3_hosts/README.md` 中的示例和故障排除。
