...
系统运行 `hostname` 并查找 `configs/3_host/<hostname>.env`。如果文件存在，在平台文件之后加载。如果不存在，系统完全跳过这一层。

```bash
# 示例：configs/3_host/my-desktop.env
USE_NVIDIA_GPU="true"
...
主机配置文件被 `.gitignore` 忽略 —— 它们是每台机器本地的，不应提交到仓库。`configs/3_host/` 目录中只有 `.gitkeep` 和 `README.md` 被追踪。
...
1. 复制 `configs/2_platforms/offline.env` → `configs/2_platforms/<your-platform>.env`
