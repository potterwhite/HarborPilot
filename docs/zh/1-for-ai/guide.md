...
以下规则**绝对执行**，不允许任何变通或自行发挥。

**文件：`docs/zh/2-progress/NEED_TO_DO.md`**
- 此文件**永久存在**。绝不删除、重命名、或 `git mv`。
- 只放活跃任务（进行中或尚未开始）。
- 所有任务全部完成后：清空任务内容，只保留文件顶部的注释行。

**存档目录：`docs/zh/2-progress/task-logs/`**
- 存档文件命名格式：`NEED_TO_DO_Archived-<英文月><日>.<年>.md`（例：`NEED_TO_DO_Archived-Mar28.2026.md`）。
- **同一天只能有一个存档文件，严格执行。**
- 归档已完成的任务时：**追加**到当天已有的存档文件末尾。
- 当天无存档文件时，新建一个。
- **严禁为同一天创建第二个存档文件**（例如加 `-aso`、`-v2` 等后缀均不允许）。
