...
3_validate_repository() {
    ...
    if [ -d ".git" ] && [ $(ls -A | grep -v '^.git$' | wc -l) -eq 0 ]; then
        ...
        cd ..
        rm -rf "/development/sdk"
        git clone "git@192.168.3.67:team_n8/n8_sdk.git" "/development/sdk" || return 1
        cd "/development/sdk"
        return 0
    fi
    ...
    if [ "$current_remote" != "git@192.168.3.67:team_n8/n8_sdk.git" ]; then
        echo "Error: Repository remote URL mismatch"
        echo "Current: $current_remote"
        echo "Expected: git@192.168.3.67:team_n8/n8_sdk.git"
        return 1
    fi
